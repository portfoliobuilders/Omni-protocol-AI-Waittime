...
import { releaseChatGptPlacement } from "../adapters/registry";
...
export class OmniWaitAd {
  ...
  constructor(
    ...
    this.host.setAttribute("aria-label", "Omni wait-time inventory");
  ...
  destroy(): void {
    releaseChatGptPlacement(this.host);
    this.host.remove();
  }
  ...
  private injectStyles(): void {
    ...
    style.textContent = `
      ...
      :host(.chatgpt-wide-rail),
      :host(.chatgpt-medium-float),
      :host(.chatgpt-narrow-dock) {
        position: fixed;
        z-index: 40;
        pointer-events: auto;
        max-height: min(240px, calc(100vh - 160px));
      }
      :host(.chatgpt-narrow-dock) .body {
        display: none;
      }
      :host(.chatgpt-narrow-dock) .card {
        padding: 10px 12px;
      }
      :host(.chatgpt-narrow-dock) .footer {
        font-size: 10px;
      }
      :host(.omni-chatgpt-rail) {
        position: fixed;
        z-index: 40;
        right: 16px;
        bottom: 120px;
  ...
  private render(): void {
    ...
    const isNarrow = this.host.classList.contains("chatgpt-narrow-dock");
    const advertiserName = !isHouse
      ? (this.ad?.creative.advertiser_name ?? "").trim()
      : "";
    const showEarning =
      this.qualifyResult &&
    ...
    if (advertiserName) {
      ...
      advertiser.textContent = truncateText(advertiserName, LIMITS.advertiserName);
    ...
    if (this.phase === "shell" || !this.ad) {
      ...
      const logoFallbackSource = advertiserName || "O";
      if (logoUrl && isHttpsUrl(logoUrl)) {
        ...
        img.addEventListener("error", () => {
          img.replaceWith(this.makeLogoFallback(logoFallbackSource));
        });
        brandRow.appendChild(img);
      } else if (!isHouse) {
        brandRow.appendChild(this.makeLogoFallback(logoFallbackSource));
      }
      ...
      if (!isNarrow && this.phase !== "compact" && this.ad.creative.body) {
      ...
      if (!isHouse && isHttpsUrl(this.ad.creative.cta_url)) {
    ...
    if (this.whyOpen) {
      ...
      panel.textContent = isHouse
        ? "This is Omni wait-time inventory. Omni did not read your conversation to choose this placement."
        : `This sponsor selected ${this.platformName} wait-time inventory. Omni did not read your conversation to choose this ad.`;
  ...
  private makeLogoFallback(source: string): HTMLElement {
    const el = document.createElement("div");
    el.className = "logo-fallback";
    const letter = source.trim().charAt(0).toUpperCase() || "O";
